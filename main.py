...
def execute(request, context):
    ...
    for watch_list_item in watch_list:
        ...
        row = aggregate_data(base_asset, quote_asset, start_timestamp, fetch_derivative_data=derivative_availale)
...
def aggregate_data(base_asset, quote_asset, start_timestamp, fetch_derivative_data = True):
    symbol = base_asset + quote_asset
    klines = binance.get_historical_klines(symbol=symbol, interval="1h", start_str=str(start_timestamp), limit=1)
    [
        open_time,
        open_price,
        high,
    ...
    # convert to USDT volume
        ...
        quote_asset_usdt_price = binance.get_ticker(symbol=quote_asset + "USDT")["lastPrice"]
        row["volume"] = float(quote_asset_volume) * float(quote_asset_usdt_price)
        row["taker_buy_volume"] = float(taker_buy_quote_asset_volume) * float(quote_asset_usdt_price)

    # only for derivative
    if fetch_derivative_data:
        oi = binance.futures_open_interest_hist(symbol=symbol, period="1h", start_str=str(start_timestamp), limit=1)
        ratio = binance.futures_top_longshort_position_ratio(symbol=symbol, period="1h", limit=1)
